<template>
	<div>
		...
		<div style="text-align: left;margin:0 0 20px 50px;">
			<el-input style='width:160px' placeholder='设备名称' v-model='search_basedevicename'></el-input>
...
<script>
	export default {
		...
		methods:{
			...
			update(row){
				this.$router.push({name:'basedeviceedit',params:{row:row.devid}})
			},
			search(){
				// 后端网址
				var url = this.baseUrl+"/baseDevice/search"
				// 传递给后端的数据
				var data = {devname:this.search_basedevicename};
				this.$axios.post(url,this.$qs.stringify(data),{
					headers: {
						'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'
					}
