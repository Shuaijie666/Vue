<template>
	<div>
		...
		<div style="text-align: left;margin:0 0 20px 50px;">
			<el-input style='width:160px' placeholder='能源类别' v-model='search_standname'></el-input>
		...
		<center>
			<el-table :data="list">
				...
				<el-table-column prop="ratio" label="系数" width="80"></el-table-column>
...
<script>
	export default {
		...
		methods:{
			...
			add(){
				// 路由跳转
				this.$router.push({path:'/standadd'})
			},
			//通过id删除
			del(sid){
				// 后端网址
			...
			update(row){
				this.$router.push({name:'standedit',params:{row:row.sid}})
			},
			search(){
				// 后端网址
				var url = this.baseUrl+"/energyStandradcoal/search"
				// 传递给后端的数据
				var data = {energetype:this.search_standname};
				this.$axios.post(url,this.$qs.stringify(data),{
					headers: {
						'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'
					}
